...
PUBLIC_ASSET_BASE_URL=http://localhost:9000/pure3d-assets
```

Use `http://minio:9000` for PocketBase because it runs inside the Docker network. Use `http://localhost:9000/pure3d-assets` for the browser-facing asset URL.
...
curl -I http://localhost:9000/pure3d-assets/
```

Open the MinIO console at `http://localhost:9001` and confirm the `pure3d-assets` bucket exists.
...
```txt
http://localhost:9000/pure3d-assets/project/13/edition/20/scene.svx.json
```
...
```sh
curl -I http://localhost:9000/pure3d-assets/project/13/edition/20/scene.svx.json
```
