...
async function main() {
  ...
  try {
    ...
    console.error('\n📝 Troubleshooting:');
    console.error('   1. Check admin account exists at http://localhost:14274/_/');
    console.error('   2. Verify credentials match .env file:');
    console.error(`      Email: ${ADMIN_EMAIL}`);
    console.error('   3. Try logging in manually at http://localhost:14274/_/\n');
    process.exit(1);
  }
  ...
  try {
    ...
    if (hasOurCollections) {
      ...
      console.log('   Alternatively, create them manually at: http://localhost:14274/_/\n');
    }
  } catch (error: any) {
  ...
  console.log('1. Create collections via Admin UI: http://localhost:14274/_/');
