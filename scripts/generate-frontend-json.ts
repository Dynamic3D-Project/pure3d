...
const collections = visibleProjects.map((project) => {
	...
	const thumbnailUrl =
		project.pubNum > 0 ? `https://editions.pure3d.eu/project/${project.pubNum}/icon.png` : '';
...
writeFileSync(join(OUTPUT_DIR, 'collections.json'), JSON.stringify(collections, null, 2));
...
writeFileSync(join(OUTPUT_DIR, 'editions.json'), JSON.stringify(editions, null, 2));
