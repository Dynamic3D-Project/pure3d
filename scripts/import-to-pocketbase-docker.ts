...
/**
 * Import MongoDB backup data into PocketBase (Docker version)
...
class PocketBaseImporter {
  ...
  async waitForPocketBase(maxRetries = 30): Promise<boolean> {
    ...
    console.log('\n❌ PocketBase did not become ready in time');
    return false;
  }

  async authenticateWithExistingAdmin(): Promise<boolean> {
    try {
      console.log('\n🔑 Authenticating with existing admin account...');
      const authResponse = await fetch(`${this.baseUrl}/api/admins/auth-with-password`, {
        method: 'POST',
      ...
      console.error('❌ Authentication failed. Please check credentials in .env');
  ...
  async checkIfDataExists(): Promise<boolean> {
    try {
      const response = await fetch(`${this.baseUrl}/api/collections/users/records?perPage=1`, {
...
async function main() {
  ...
  console.log('🚀 Starting PocketBase Import (Docker)');
  ...
  const isReady = await importer.waitForPocketBase();
  if (!isReady) {
    process.exit(1);
  }

  // Authenticate with existing admin
  const adminReady = await importer.authenticateWithExistingAdmin();
  if (!adminReady) {
  ...
  const dataExists = await importer.checkIfDataExists();
  if (dataExists) {
    console.log('\n⚠️  Data already exists in PocketBase. Skipping import.');
    console.log('   To re-import, delete the data first or reset PocketBase.');
    process.exit(0);
  }
